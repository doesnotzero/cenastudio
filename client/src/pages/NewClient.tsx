...
import { api, startCheckout, type ClientAllowance } from "@/lib/api";
...
function NewClientContent() {
  ...
  const [allowance, setAllowance] = useState<ClientAllowance | null>(null);

  useEffect(() => {
    api.clients.allowance().then(setAllowance).catch(() => setAllowance(null));
  }, []);
    ...
    <div className="min-h-screen bg-frame-black text-frame-white font-frame-body">
      ...
      <div className="max-w-3xl mx-auto px-6 py-10">
        ...
        {allowance && allowance.limit !== null && (
          <div className="mb-5 flex flex-wrap items-center justify-between gap-3 border border-frame-gray-3 bg-frame-gray-1/30 px-4 py-3">
            <p className="font-frame-mono text-[0.65rem] uppercase text-frame-gray-light">
              Plano {allowance.planId}: {allowance.used} de {allowance.limit} clientes
            </p>
            {!allowance.canCreate && (
              <button type="button" onClick={() => void startCheckout("pro")} className="frame-btn-primary">
                Aumentar limite
              </button>
            )}
          </div>
        ...
        <form onSubmit={handleSubmit} className="border border-frame-gray-3 bg-frame-gray-1/10 p-6">
          ...
          <div className="flex gap-3 mt-6 pt-4 border-t border-frame-gray-3">
            ...
            <button type="submit" disabled={isSubmitting || !data.name.trim() || allowance?.canCreate === false} className="frame-btn-primary flex-1">
