...
# Verificar provedores de auth disponíveis
curl https://cenastudio.com.br/api/auth/providers
```
...
- Email/senha com `frame_token` é o caminho primário de auth. Em produção, os usuários ficam no Supabase Postgres via Prisma. Supabase Auth Admin, GitHub OAuth e workspace bootstrap são sincronizações tolerantes e não devem bloquear login/admin/demo.
...
- Antes de promover deploy, rode smoke de auth: login admin, login demo, registro público, criação admin, `/api/auth/me`, logout, `/api/auth/providers` e GitHub configurado/desconfigurado.
- No smoke comercial, valide: sexto cliente no Free retorna `402`; Studio recém-cadastrado fica `pending`; `/api/checkout/sync-session` ativa somente sessão paga; após ativação, rotas operacionais deixam de retornar `402`.
- `ADMIN_DEFAULT_PASSWORD` e `DEMO_USER_PASSWORD` precisam ter no mínimo 12 caracteres em produção. O seed reconcilia essas senhas em contas existentes.
- `GITHUB_CALLBACK_URL` aponta para `/api/auth/github/callback`; `CLIENT_ORIGIN` define o redirecionamento final para o frontend.
